...
// Old base colors + new fiberClass & headerGlow from new
...
// Old STATUS_BADGE colors, new badge-crystal class from new
...
// ── Sub-component: Trading preview strip ──────────────────────────────────────
...
function TradingPreview({
  ...
  return (
    <div className="flex items-center gap-2 mt-1.5 ml-4 flex-wrap min-w-0">
      ...
      {dir && (
        <span
          className="data-chip inline-flex items-center gap-1"
          style={{
            fontSize: 9,
            fontWeight: 700,
      ...
      {hasPnl && (
        <span
          className={cn(
            "data-chip inline-flex items-center gap-1",
            isWin ? "chip-win" : isLoss ? "chip-loss" : "",
          )}
          style={{
            fontSize: 9,
            fontWeight: 700,
...
// ── Sub-component: Expense preview strip ──────────────────────────────────────
...
function ExpensePreview({
  ...
  return (
    <div className="flex items-center gap-2 mt-1.5 ml-4 flex-wrap min-w-0">
      ...
      {hasAmount && (
        <span
          className="data-chip chip-loss inline-flex items-center gap-1"
          style={{
            fontSize: 9,
            fontWeight: 700,
            fontFamily: "monospace",
            ...
            borderRadius: 4,
            padding: "2px 6px",
            boxShadow: "0 0 8px rgba(244,114,182,0.1)",
          }}
        >
          <DollarSign size={7} />
...
// ── Sub-component: General preview strip ──────────────────────────────────────
  ...
  return (
    <div className="flex items-center gap-2 mt-1.5 ml-4 min-w-0">
      ...
      {page.tags?.slice(0, 2).map((pt) => (
        <span
          key={pt.tagId}
          className="data-chip text-[9px] px-1.5 py-0.5 rounded-full border"
          style={{
            color: pt.tag.color,
            borderColor: pt.tag.color + "40",
            background: pt.tag.color + "15",
            boxShadow: `0 0 6px ${pt.tag.color}20`,
          }}
        >
          {pt.tag.name}
        </span>
...
export default function PageList() {
  ...
  // Track seen ids for stagger entry animation (from new)
  ...
  const handleStatusCycle = async (
    ...
    e.stopPropagation();
    const order = ["active", "done", "archived"];
  ...
  return (
    // Old: w-64 h-screen flex flex-col border-r border-white/10 bg-black/20
    <div className="w-64 h-screen flex flex-col border-r border-white/10 bg-black/20">
      {/* ── Header — old layout, new headerGlow shadow ── */}
      <div
      ...
      >
        <div className="flex items-center gap-2">
          {/* Old dot with new halo-ring animation wrapper */}
          <span
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 space-y-0.5">
        ...
        {sectionPages.map((page, i) => {
          ...
          const statusBadge = STATUS_BADGE[page.status] ?? STATUS_BADGE.active;
          // New: stagger fragmentIn only for first render
          ...
          return (
            <div
              ...
              // Old class structure, old active bg/border
              className={cn(
              ...
              style={{
                ...
                boxShadow: isActive
                  ...
                  : undefined,
                // New: fragmentIn stagger on first appearance
            ...
            >
              {/* New: neon fiber bar (active only) */}
              {isActive && (
                <span
              ...
              {/* Row 1: icon + title + menu — old layout */}
              ...
              {/* Row 2: Status badge — old layout, new badge-crystal class */}
              ...
              {/* Row 3: Type-specific data preview — old layout, new data-chip classes */}
      ...
      {/* ── Context menu — new ctx-menu class + separator, old color values ── */}
      {menuId && (
        <div
          ref={menuRef}
          className="ctx-menu"
            ...
            return (
              <button
                ...
                style={{
                  ...
                  transition:
                    "background 0.1s ease, padding-left 0.14s cubic-bezier(0.16,1,0.3,1), color 0.1s",
                }}
                onMouseEnter={(e) => {
